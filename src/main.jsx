...
const navItems = [
  ...
  { id: 'add', label: 'إضافة مصروف', icon: FilePlus2 },
...
function Dashboard({ user, db, setPage }) {
  ...
  const balance = calculateCustodyBalance(visibleAllocations, visibleTransactions);
...
function CustodyPage({ user, db, notify }) {
  const [editing, setEditing] = useState(null);
  const allocations = getVisibleAllocations(user, db);
  const visibleTransactions = getVisibleTransactions(user, db);
  const balance = calculateCustodyBalance(allocations, visibleTransactions);

  const receive = (allocation) => {
  ...
  const remove = (id) => {
    if (!confirm('هل تريد حذف العهدة؟')) return;
    deleteRecord('custodyAllocations', id);
    notify('تم حذف العهدة');
  };

  const save = (payload) => {
    const saved = upsertRecord('custodyAllocations', {
      ...payload,
    ...
    const hasProjectLink = latest.userProjects.some((link) => link.userId === saved.userId && link.projectId === saved.projectId);
    if (!hasProjectLink) {
      saveDatabase({
        ...latest,
        userProjects: [
          ...latest.userProjects,
          { id: `up-${Date.now()}`, userId: saved.userId, projectId: saved.projectId }
        ]
      });
    }
    setEditing(null);
    notify(payload.id ? 'تم تعديل العهدة' : 'تم إضافة العهدة للمستخدم');
  };

  return (
    <Page title="العهدة" action={user.role === 'admin' && <button className="btn-primary" onClick={() => setEditing({ status: 'pending' })}><Plus size={18} /> عهدة جديدة</button>}>
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Stat title="إجمالي العهدة" value={formatCurrency(balance.assignedTotal)} />
        <Stat title="المستلم" value={formatCurrency(balance.receivedTotal)} />
        <Stat title="بانتظار الاستلام" value={formatCurrency(balance.pendingTotal)} />
        <Stat title="المتبقي بعد المصروفات" value={formatCurrency(balance.remainingTotal)} />
      </div>
      <CardsGrid>
        {allocations.map((allocation) => {
          const scopeTransactions = filterCustodyScope(db.transactions, allocation.projectId, allocation.userId);
          const scopeAllocations = filterCustodyScope(db.custodyAllocations || [], allocation.projectId, allocation.userId);
          const scopeBalance = calculateCustodyBalance(scopeAllocations, scopeTransactions);
          return (
            <article className="card" key={allocation.id}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-lg font-extrabold text-brand-navy">{nameById(db.projects, allocation.projectId, 'projectName')}</h3>
                  <p className="text-sm text-slate-500">{nameById(db.users, allocation.userId, 'name')}</p>
                </div>
                <Badge>{allocation.status === 'received' ? 'تم الاستلام' : 'بانتظار الاستلام'}</Badge>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                <MiniMetric title="قيمة العهدة" value={formatCurrency(allocation.amount)} />
                <MiniMetric title="رصيد المستخدم بالمشروع" value={formatCurrency(scopeBalance.remainingTotal)} />
              </div>
              {allocation.notes && <p className="mt-3 text-sm text-slate-600">{allocation.notes}</p>}
              <div className="mt-4 flex flex-wrap gap-2">
                {user.role === 'user' && allocation.status === 'pending' && <button className="btn-primary" onClick={() => receive(allocation)}>تم الاستلام</button>}
                {user.role === 'admin' && <button className="btn-ghost" onClick={() => setEditing(allocation)}><Pencil size={17} /> تعديل</button>}
                {user.role === 'admin' && <button className="btn-danger" onClick={() => remove(allocation.id)}><Trash2 size={17} /> حذف</button>}
              </div>
            </article>
          );
        })}
      </CardsGrid>
      {!allocations.length && <EmptyState text="لا توجد عهدة حتى الآن" />}
      {editing && <CustodyModal initial={editing} db={db} onClose={() => setEditing(null)} onSave={save} />}
    </Page>
  );
}
...
function ReportsPage({ user, db }) {
  ...
  const reportAllocations = getVisibleAllocations(user, db).filter((allocation) => {
    if (filters.projectId && allocation.projectId !== filters.projectId) return false;
    if (filters.userId && allocation.userId !== filters.userId) return false;
    return true;
  });
  const custodyBalance = calculateCustodyBalance(reportAllocations, all);
    ...
    <Page title="التقارير" action={<button className="btn-primary" onClick={() => printReport('printable-report', 'تقرير منظومة عهدة مقاولات')}><Printer size={18} /> تحميل PDF</button>}>
      ...
      <section id="printable-report" className="mt-5 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-soft">
        ...
        <div className="summary mt-4 grid gap-3 sm:grid-cols-3">
          <Stat title="العهدة المستلمة" value={formatCurrency(custodyBalance.receivedTotal)} />
          <Stat title="إجمالي العهدة" value={formatCurrency(totals.custodyTotal)} />
          <Stat title="المتبقي" value={formatCurrency(custodyBalance.remainingTotal)} />
...
function TransactionEditor({ user, db, notify, initial = {}, embedded = false }) {
  ...
  const projectAllocations = filterCustodyScope(db.custodyAllocations || [], form.projectId, form.userId);
  const projectTransactions = filterCustodyScope(db.transactions, form.projectId, form.userId).filter((transaction) => transaction.id !== form.id);
  const balanceBeforeCurrent = calculateCustodyBalance(projectAllocations, projectTransactions);
  const remainingAfterSave = balanceBeforeCurrent.remainingTotal - total;

  const submit = async (event) => {
    event.preventDefault();
    if (balanceBeforeCurrent.receivedTotal <= 0) {
      notify('لا توجد عهدة مستلمة لهذا المشروع');
      return;
    }
    if (remainingAfterSave < 0) {
      notify('قيمة المصروف أكبر من الرصيد المتبقي للعهدة');
      return;
    }
  ...
  return (
    <form onSubmit={submit} className={embedded ? '' : 'rounded-2xl border border-slate-200 bg-white p-4 shadow-soft'}>
      ...
      <div className="mt-3 grid gap-3 rounded-xl bg-brand-mint p-4 text-brand-navy md:grid-cols-3">
        <strong>الإجمالي: {formatCurrency(total)}</strong>
        <strong>الرصيد قبل الحفظ: {formatCurrency(balanceBeforeCurrent.remainingTotal)}</strong>
        <strong className={remainingAfterSave < 0 ? 'text-red-700' : ''}>المتبقي بعد الحفظ: {formatCurrency(remainingAfterSave)}</strong>
      </div>
...
function CustodyModal({ initial, db, onClose, onSave }) {
  const [form, setForm] = useState({
    projectId: db.projects[0]?.id || '',
    userId: db.users.find((user) => user.role === 'user')?.id || db.users[0]?.id || '',
    amount: 0,
    status: 'pending',
    notes: '',
    ...initial
  ...
  return (
    <Modal title={form.id ? 'تعديل عهدة' : 'إضافة عهدة'} onClose={onClose}>
      <form onSubmit={(e) => { e.preventDefault(); onSave(form); }}>
        <Select label="المشروع" value={form.projectId} onChange={(projectId) => setForm({ ...form, projectId })} options={db.projects.map((project) => [project.id, project.projectName])} />
        <Select label="المستخدم" value={form.userId} onChange={(userId) => setForm({ ...form, userId })} options={db.users.map((user) => [user.id, `${user.name} - ${user.role === 'admin' ? 'مسؤول' : 'مستخدم'}`])} />
        <Label text="قيمة العهدة">
        ...
        <Select label="الحالة" value={form.status} onChange={(status) => setForm({ ...form, status })} options={[['pending', 'بانتظار الاستلام'], ['received', 'تم الاستلام']]} />
        <Label text="ملاحظات">
...
function MiniMetric({ title, value }) {
  return <div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">{title}</p><strong className="mt-1 block text-brand-navy">{value}</strong></div>;
}
...
function getVisibleAllocations(user, db) {
  const allocations = db.custodyAllocations || [];
  return user.role === 'admin' ? allocations : allocations.filter((allocation) => allocation.userId === user.id);
}
